...
## v4.4（专业萃取师深度增强）— 2026-09-04

把 references 里的专业萃取方法论精华融入 SKILL.md 主流程，让使用者直接感受到专业萃取师的深度：
1. **定阶段新增「萃取自己 vs 萃取别人」适配**：萃取自己用「改变人生的话题五分类」破冰+升维引导（具体经历→方法论主题）；萃取别人用 STAR 还原+访谈技巧（多问动作少问原因）。
2. **萃阶段大幅增强**：
   - 新增「判断回答类型」（7 种类型：观点/大而全/具体做法/成功结果/失败踩坑/心法理念/抱怨环境 → 对应优先追问方向）
   - 新增「对抗认知闭合自动触发」（4 种信号 → 回拉话术）
   - 新增「灵魂三问连挖」（褚冬彪：差异点/边界点/迁移点）
   - 新增「四问深挖框架」（陈晓燕：识别难点→挖掘做法→反思意图→探询可能）
   - 新增「八大方向判停条件」
   - 新增「一句话 SOP 收敛 + 三问收口」（命名/最难教/遗漏）
3. Quick Start 同步增强。
方法论零改动。

## v4.3（萃取体验优化）— 2026-09-04

基于真实萃取演练的用户反馈，优化三处体验：
1. **追问菜单支持多选**：被萃取者可选一个方向，也可一次性回答多个；若只选一个，后续会继续补问其他方向，不会漏掉（Quick Start / 核心原则第9条 / 主流程萃 / 操作总则四处同步修改）。
2. **新增"成果形态引导"环节**：萃取完成后主动询问用户想把经验做成什么（课程 / 手册 / 培训材料 / 工具包 / 变现产品 / 组织资产），根据选择匹配封装模板。
3. **质量红线新增第 11 条**：最终产出的萃取报告/成果物中，必须按"定→萃→验→用"顺序说明每一步使用了哪些方法论框架（STAR / SCQA / ORID / 鱼骨图 / 4D / SBP / 三阶萃取 / 训战六步 / GRAI / CSS 复盘 / 一纸禅等）。
方法论零改动。

## v4.2（真实萃取演练 + 双 demo）— 2026-09-04

完成第 2 层真实萃取演练（主题：社群从 0→1 高质量增长，萃取对象：青影星球AI 社群发起人），经 3 轮对话式动态萃取产出第二个完整 demo 技能包 `examples/demo-社群从0到1增长-打法包/`：
- 3 张牛招卡：朋友圈递进式冷启动三连发 / 聚合式日报五模块 / 嘉宾场景高信任导流
- skill-pack.yaml（manifest）+ test-prompts.json（5 例评测）+ DIGEST.md + 01-萃取过程记录.md
- 技能包校验 PASS（必要文件/manifest/打法卡/评测用例）
- 有真实数据支撑：朋友圈截图（连续 3 天）、800 天日报零断更、现场一次二三十人进群、大几十人/大几十个博主转载
更新 `SELF-TEST-REPORT.md` 第 2 层为真实演练结果。方法论零改动。
