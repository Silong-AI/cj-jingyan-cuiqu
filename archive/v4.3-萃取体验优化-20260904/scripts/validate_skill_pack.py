...
检查项:
    1. 必要文件齐全: README.md / skill-pack.yaml / DIGEST.md / test-prompts.json / 打法库/
    2. skill-pack.yaml 为有效 YAML 且含 pack_id / capabilities
    3. 打法库每张卡含: capability_id / level / 一句话SOP / 适用 / 验证
    4. test-prompts.json 为有效 JSON 且 tests 非空
    5. manifest 中声明的打法文件实际存在
    ...
    try:
        ...
        if not isinstance(caps, list) or not caps:
            ...
            for c in caps:
                if not c.get("id") or not c.get("file"):
                    fails.append("存在缺少 id 或 file 的打法声明")
                elif not os.path.isfile(os.path.join(root, c["file"])):
                    fails.append(f"manifest 声明的打法文件不存在: {c['file']}")
        # ★v2.2 JSON Schema 深度校验
        ...
        if os.path.isfile(schema_path):
            try:
                ...
                print("[info] jsonschema 未安装，跳过 JSON Schema 校验（基础字段已检查）")
            except jsonschema.ValidationError as e:
                fails.append(f"skill-pack.yaml 不符合 Schema: {e.message} (位置: {list(e.path)})")
            except Exception as e:
                fails.append(f"Schema 校验异常: {e}")
    except ImportError:
        # 无 yaml 库时仅做基础存在性检查
...
def check_cards(root):
    ...
    if not os.path.isdir(dry):
        return ["打法库/ 目录不存在"]
    md_files = [f for f in os.listdir(dry) if f.endswith(".md") and f != "打法索引.md"]
    if not md_files:
        return ["打法库/ 为空"]
    for f in md_files:
