...
# -*- coding: utf-8 -*-
"""
generate_index.py — CJ·经验萃取 干法库索引自动生成器
扫描技能包的 干法库/*.md，读取每张干法卡 frontmatter 注释与标题，
自动生成"干法索引.md"总览表（ID | 干法名 | 层级 | 验证 | 一句话）。

用法:
    python generate_index.py <技能包目录>

输出:
    写入 <技能包目录>/干法库/干法索引.md（若已存在则覆盖更新）
"""
import os
import re
import sys
...
def parse_card(path):
    """从干法卡提取: id / level / verification / 标题 / 一句话"""
    try:
...
def main():
    ...
    if not os.path.isdir(dry):
        print(f"FAIL: 未找到干法库目录 {dry}")
        return 1

    cards = []
    for f in sorted(os.listdir(dry)):
        if not f.endswith(".md") or f == "干法索引.md":
            continue
    ...
    out = os.path.join(dry, "干法索引.md")
    with open(out, "w", encoding="utf-8") as fh:
        fh.write("\n".join(lines) + "\n")
    print(f"[OK] 已生成 {out}（{len(cards)} 张干法卡）")
    return 0
