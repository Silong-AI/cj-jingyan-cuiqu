...
2. **稳定标识**：每条干法有固定 `capability_id`，跨版本不变（改名只增新 ID，不删旧 ID）。
3. **前端可触发**：每个"干法卡"或"入口"的 SKILL.md 必须遵循 Agent Skills frontmatter 规范（`name` + `description` 触发词）。
...
干法库的"干法索引.md"不手写，由 `scripts/generate_index.py` 自动生成：扫描 `干法库/*.md`，读取每张卡的 frontmatter 注释（`capability_id` / `level`）与"一句话"，汇总成总览表。每次新增/修改干法卡后重跑一次即可，保证索引与卡片不脱节。

## 三、干法卡 → 能力卡（对齐 cangjie 的 RIA 卡）

每张干法卡建议对齐社区"能力卡"结构，让第三方可识别其意图与边界：
...
- 每个干法卡的 frontmatter 遵循 Agent Skills 规范：`name` + `description`（含明确触发词 + "不在一般性问题上自动触发"防呆）。
...
| cangjie-skill | 书/视频的方法论 | CJ 的干法卡对齐其 RIA 能力卡 + verified 清单 |
| darwin-skill | 进化任意 skill | CJ 技能包附 test-prompts.json，可直接喂 darwin 进化 |
| **CJ·经验萃取** | **组织内高手的干法** | **产出：团队技能包（给组织用）** |
