# CJ·经验萃取（Experience Extraction）v3.1

> 把「人会做但说不清」的高手经验，变成「看得见、学得会、用得着、验证过」的组织资产或个人方法论。

**一句话定位**：nuwa 蒸「人」、cangjie 蒸「书/视频」、darwin 进化 skill——**CJ 蒸「组织内的高手」，把销冠/专家/老员工的隐性干法，转成团队可训练、可验证、可进化的技能包**。
...
- **三重验证门禁**：每条干法过 V1 跨域复现 / V2 预测力 / V3 独特性，不过就降级淘汰。
- **落地验证协议**：A/B 对照、SOP 可用性测试（新人照着做能否 60 分）、冲突仲裁、经验过期检测。
- **产出即落地**：组织级萃取直接产出「可安装团队技能包」（干法库 + 成果物 + 训练包 + 验证包 + DIGEST）。

**适用场景**
- 个人经验萃取：萃取某个人（专家/销冠/老员工/自己）在特定领域的成功干法 → 个人方法论/经验手册。
...
```
cj-jingyan-cuiqu/
├── SKILL.md                    # 主文档：入口分流 + 主流程 + 质量红线（v3.1）
├── README.md                   # 本文件：给使用者的说明
├── CHANGELOG.md                # 版本变更记录
...
| v2.2 | 制作质量 | 萃取师 Voice Check、失败降级表、JSON Schema 校验、干法索引生成、回炉上限、知情同意 |
